package dev.liquidnetwork.liquidpractice.match;
...
import dev.liquidnetwork.liquidpractice.match.events.MatchEndEvent;
import dev.liquidnetwork.liquidpractice.match.events.MatchStartEvent;
import dev.liquidnetwork.liquidpractice.match.task.*;
...
public abstract class Match {
    ...
    private final dev.liquidnetwork.liquidpractice.queue.Queue queue;
    ...
    public Match(dev.liquidnetwork.liquidpractice.queue.Queue queue, Kit kit, Arena arena, QueueType queueType) {
        this.queue = queue;
        this.kit = kit;
        this.arena = arena;
    ...
    public static void preload() {
        new MatchPearlCooldownTask().runTaskTimerAsynchronously(LiquidPractice.getInstance(), 2L, 2L);
        new MatchSnapshotCleanupTask().runTaskTimerAsynchronously(LiquidPractice.getInstance(), 20L * 5, 20L * 5);
        new BukkitRunnable(){
            @Override
            public void run() {
        ...
        }.runTaskTimer(LiquidPractice.getInstance() , 20 , 20);
    }
    ...
    public static void cleanup() {
        for (Match match : matches) {
            ...
            if (!LiquidPractice.getInstance().isDisabling()) {
                Bukkit.getScheduler().runTaskLaterAsynchronously(LiquidPractice.getInstance(), new MatchBoxUHCTask(match), 5L);
            } else {
    ...
    public void start() {
        ...
        if (!this.isSumoMatch() || !this.isSoloMatch() || !isHCFMatch() || !this.isTheBridgeMatch()) {
            if (this.isFreeForAllMatch() || this.isTeamMatch() || this.isSumoTeamMatch()) {
                for ( String string : LiquidPractice.getInstance().getMessagesConfig().getStringList("Match.Start-Message.Team")) {
                    this.broadcastMessage(CC.translate(this.replace(string)));
                }
            }
        }
        if (isHCFMatch()) {
            for ( String string : LiquidPractice.getInstance().getMessagesConfig().getStringList("Match.Start-Message.HCF")) {
        ...
        if (getKit() != null) {
            if (getKit().getGameRules().isWaterkill() || getKit().getGameRules().isParkour() || getKit().getGameRules().isSumo()) {
                matchWaterCheck = new MatchWaterCheckTask(this).runTaskTimer(LiquidPractice.getInstance(), 60L, 20L);
            }
        }

        task = new MatchStartTask(this).runTaskTimer(LiquidPractice.getInstance(), 20L, 20L);
        for (Player shooter : getPlayers()) {
            ...
            }.runTaskTimerAsynchronously(LiquidPractice.getInstance(), 0L, 5L);
        }
    ...
    public void end() {
        ...
        new MatchResetTask(this).runTask(LiquidPractice.getInstance());
        ...
        Bukkit.getScheduler().runTaskLaterAsynchronously(LiquidPractice.getInstance(), () -> getPlayers().forEach(player -> ((CraftPlayer) player).getHandle().getDataWatcher().watch(9, (byte) 0)), 10L);
        Bukkit.getScheduler().runTaskLaterAsynchronously(LiquidPractice.getInstance(), () -> getPlayers().forEach(player -> ((CraftPlayer) player).getHandle().getDataWatcher().watch(9, (byte) 0)), 20L);
    }
    ...
    public void handleDeath(Player deadPlayer, Player killerPlayer, boolean disconnected) {
        ...
        if (!isSumoMatch() && !isSumoTeamMatch() && !isTheBridgeMatch()) {
            if (canEnd()) {
                ...
                }.runTaskLaterAsynchronously(LiquidPractice.getInstance(), 20L);
            }
        } else {
            if (canEnd()) {
                end();
