...
import dev.liquidnetwork.liquidpractice.util.PlayerSnapshot;
import dev.liquidnetwork.liquidpractice.util.PlayerUtil;
import dev.liquidnetwork.liquidpractice.util.chat.CC;
import dev.liquidnetwork.liquidpractice.util.chat.Clickable;
...
import dev.liquidnetwork.liquidpractice.event.types.sumo.task.SumoRoundEndTask;
import dev.liquidnetwork.liquidpractice.event.types.sumo.task.SumoRoundStartTask;
...
public class Sumo {
	...
	public List<String> getLore() {
		...
		Sumo sumo = LiquidPractice.getInstance().getSumoManager().getActiveSumo();
	...
	public void setEventTask(SumoTask task) {
		...
		if (eventTask != null) {
			eventTask.runTaskTimer(LiquidPractice.getInstance(), 0L, 20L);
		}
	}
	...
	public void end() {
		LiquidPractice.getInstance().getSumoManager().setActiveSumo(null);
		LiquidPractice.getInstance().getSumoManager().setCooldown(new Cooldown(60_000L * 10));
	...
	public void onJoin(Player player) {
	 Profile.setKb(player, LiquidPractice.getInstance().getSumoManager().getSumoKnockbackProfile());
	}
	...
	public void onRound() {
		...
		playerA.teleport(LiquidPractice.getInstance().getSumoManager().getSumoSpawn1());
		playerB.teleport(LiquidPractice.getInstance().getSumoManager().getSumoSpawn2());
	...
	public void onDeath(Player player) {
		...
		winner.getPlayer().teleport(LiquidPractice.getInstance().getSumoManager().getSumoSpectator());
	...
	public void addSpectator(Player player) {
		...
		player.teleport(LiquidPractice.getInstance().getSumoManager().getSumoSpawn1());
	}
