...
import dev.liquidnetwork.liquidpractice.match.events.MatchEvent;
...
public class ProfileListener implements Listener {
    ...
    public void onPlayerMatchStart(MatchEvent e) {
        if (e instanceof MatchStartEvent) {
            ...
            Bukkit.getScheduler().runTaskLaterAsynchronously(LiquidPractice.getInstance(), () -> match.getPlayers().forEach(player -> {
    ...
    public void onPlayerJoinEvent(PlayerJoinEvent event) {
        ...
        TaskUtil.runAsync(() -> {
            ...
            if (!LiquidPracticeCache.getPlayerCache().containsKey(player.getName())) {
                LiquidPracticeCache.getPlayerCache().put(player.getName(), player.getUniqueId());
            }
            try {
                profile.load();
            } catch (Exception e) {
        ...
        }.runTaskLaterAsynchronously(LiquidPractice.getInstance(), 5L);
    ...
    public void onPlayerQuitEvent(PlayerQuitEvent event) {
        ...
        if (profile.getRematchData() != null) {
            Player target = LiquidPractice.getInstance().getServer().getPlayer(profile.getRematchData().getTarget());
